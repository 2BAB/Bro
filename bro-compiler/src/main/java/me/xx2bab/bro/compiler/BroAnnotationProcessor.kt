...
class BroAnnotationProcessor : AbstractProcessor() {
    ...
    override fun init(processingEnv: ProcessingEnvironment) {
        ...
        for (processor in processors) {
            supportedAnnotations.addAll(processor.supportedAnnotationTypes)
        }

        singleModuleCollector = SingleModuleCollector(
                processors,
                processingEnv,
                FileUtils.getDefault(),
                moduleName!!,
        ...
        if (moduleBuildType == ModuleType.APPLICATION) {
            ...
            genOutputs.appAptGenDirectory = File(appAptGenPath)
            genOutputs.broBuildDirectory = File(moduleBroBuildDir)
